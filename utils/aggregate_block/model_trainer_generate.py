...
def generate_cls_model(
    ...
    **kwargs,
    ...
    if model_name == 'resnet18':
        ...
        logging.debug('Make sure you want PreActResNet34, which is NOT resnet34.')
        from models.preact_resnet import PreActResNet34
        if kwargs.get("pretrained", False):
            logging.warning("PreActResNet34 pretrained on cifar10, NOT ImageNet!")
            net_from_cifar10 = PreActResNet34()  # num_classes = num_classes)
            net_from_cifar10.load_state_dict(
                torch.load("../resource/trojannn/clean_preactresnet34.pt", map_location="cpu"
                           )['model_state_dict']
            )
        ...
        logging.debug('Make sure you want PreActResNet50, which is NOT resnet50.')
        from models.preact_resnet import PreActResNet50
        if kwargs.get("pretrained", False):
            logging.warning("PreActResNet50 pretrained on cifar10, NOT ImageNet!")
            net_from_cifar10 = PreActResNet50()  # num_classes = num_classes)
            net_from_cifar10.load_state_dict(
                torch.load("../resource/trojannn/clean_preactresnet50.pt", map_location="cpu"
                           )['model_state_dict']
            )
        ...
        logging.debug('Make sure you want PreActResNet101, which is NOT resnet101.')
        from models.preact_resnet import PreActResNet101
        if kwargs.get("pretrained", False):
            logging.warning("PreActResNet101 pretrained on cifar10, NOT ImageNet!")
            net_from_cifar10 = PreActResNet101()  # num_classes = num_classes)
            net_from_cifar10.load_state_dict(
                torch.load("../resource/trojannn/clean_preactresnet101.pt", map_location="cpu"
                           )['model_state_dict']
            )
        ...
        logging.debug('Make sure you want PreActResNet152, which is NOT resnet152.')
        from models.preact_resnet import PreActResNet152
        if kwargs.get("pretrained", False):
            logging.warning("PreActResNet152 pretrained on cifar10, NOT ImageNet!")
            net_from_cifar10 = PreActResNet152()  # num_classes = num_classes)
            net_from_cifar10.load_state_dict(
                torch.load("../resource/trojannn/clean_preactresnet152.pt", map_location="cpu"
                           )['model_state_dict']
            )
        ...
        if kwargs.get("pretrained", False):
            ...
            net = PreActResNet18(num_classes=num_classes)
    elif model_name == "blind_resnet18":
        from models.blind_resnet import resnet18
        net = resnet18(num_classes=num_classes, **kwargs)
