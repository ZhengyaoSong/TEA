...
def get_num_classes(dataset_name : str) -> int:
    ...
    if dataset_name in ["mnist", "cifar10","EuroSAT_RGB","MSTAR"]:
        num_classes = 10
    elif dataset_name == "gtsrb":
        num_classes = 43
    elif dataset_name == "NWPU":
        num_classes = 45
    elif dataset_name == "RSSCN7":
        num_classes = 7
...
def get_dataset_normalization(dataset_name):
    ...
    if dataset_name == "cifar10":
        ...
        dataset_normalization = (transforms.Normalize([0.3444, 0.3803, 0.4078], [0.0914, 0.0651, 0.0552]))
    elif dataset_name == 'MSTAR':    
        dataset_normalization = (transforms.Normalize([0.2378, 0.2378, 0.2378], [0.1500, 0.1500, 0.1500]))
    elif dataset_name == 'NWPU':    
        dataset_normalization = (transforms.Normalize([0.3680, 0.3810, 0.3436], [0.1453, 0.1356, 0.1320]))
        # dataset_normalization = transforms.Normalize([0, 0, 0], [1, 1, 1])
    elif dataset_name == 'RSSCN7':    
        dataset_normalization = (transforms.Normalize([0.3739, 0.3840, 0.3440], [0.1402, 0.1286, 0.1239]))
...
def speed_up_save(
    ...
    '''
    logging.debug(f"save the speed up matrix data for {'train' if train else 'test'} at {dataset_path}")
    x_list = []
    y_list = []
    for x, y in dataset:
...
def speed_up_load(
    ...
    if train and {"train_x.npy", "train_y.npy"}.issubset(os.listdir(dataset_path)):
        logging.debug(f"load speed up matrix for train data, at {dataset_path}")
        ...
        logging.debug(f"load speed up matrix for test data, at {dataset_path}")
...
def dataset_and_transform_generate(args):
    ...
    if (train_dataset_without_transform is None) or (test_dataset_without_transform is None):
        ...
        elif args.dataset == "EuroSAT_RGB":
            from dataset.EuroSAT_RGB import EuroSAT_RGB
            train_dataset_without_transform = EuroSAT_RGB(args.dataset_path,
                                                      train=True,
                                                    )
            test_dataset_without_transform = EuroSAT_RGB(args.dataset_path,
                                                      train=False,
                                                    )
        elif args.dataset == "NWPU":
            from dataset.NWPU import NWPU
            train_dataset_without_transform = NWPU(args.dataset_path,
                                                      train=True,
                                                    )
            test_dataset_without_transform = NWPU(args.dataset_path,
                                                      train=False,
                                                    )
        elif args.dataset == "MSTAR":
            from dataset.MSTAR import MSTAR
            train_dataset_without_transform = MSTAR(args.dataset_path,
                                                      train=True,
                                                    )
            test_dataset_without_transform = MSTAR(args.dataset_path,
                                                      train=False,
                                                    )
        elif args.dataset == "RSSCN7":
            from dataset.RSSCN7 import RSSCN7
            train_dataset_without_transform = RSSCN7(args.dataset_path,
                                                      train=True,
                                                    )
            test_dataset_without_transform = RSSCN7(args.dataset_path,
                                                      train=False,
                                                    )
