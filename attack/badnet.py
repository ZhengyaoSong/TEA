...
from utils.trainer_cls import ModelTrainerCLS_v2, class_wise_acc, BackdoorModelTrainer, all_acc
...
class BadNet(NormalCase):
    ...
    def stage2_training(self):
        ...
        #     train_dataset=bd_train_dataset,
        #     test_dataset_dict={
        ...
        #     epoch_loss, epoch_predict, epoch_label, epoch_original_index, epoch_poison_indicator, epoch_original_targets = trainer.train_one_epoch(verbose=1)
        #
        #     print(epoch_loss, epoch_predict, epoch_label, epoch_original_index, epoch_poison_indicator, epoch_original_targets)
        #
        #     poison_indicator = torch.where(epoch_poison_indicator == 1)[0]
        #     print(all_acc(epoch_predict,epoch_label))
        #     print(class_wise_acc(epoch_predict, epoch_label, list(range(10))))
        #     print(all_acc(epoch_predict[poison_indicator], epoch_label[poison_indicator]))
        #     print(class_wise_acc(epoch_predict[poison_indicator], epoch_label[poison_indicator], list(range(10))))
        #
        #     trainer.test_all_inner_dataloader()
        #     trainer.agg_save_dataframe()
        # trainer.agg_save_summary()

        from torch.utils.data.dataloader import DataLoader
        trainer.train_with_test_each_epoch_on_mix(
            DataLoader(bd_train_dataset_with_transform, batch_size = args.batch_size, shuffle=True, drop_last=True, pin_memory=args.pin_memory, num_workers=args.num_workers, ),
            DataLoader(clean_test_dataset_with_transform, batch_size = args.batch_size, shuffle=False, drop_last=False, pin_memory=args.pin_memory, num_workers=args.num_workers, ),
            DataLoader(bd_test_dataset_with_transform, batch_size = args.batch_size, shuffle=False, drop_last=False, pin_memory=args.pin_memory, num_workers=args.num_workers, ),
            args.epochs,
            criterion=criterion,
            optimizer=optimizer, 
            scheduler=scheduler,
